...
class perhitungan_Model extends CI_Model
{
    ...
    public function get_AllKaryawan($tahun)
    {
        // return $this->db->get_where('penilaian', ['id_periode' => $tahun])->result_array();
        $query = "SELECT `id_penilaian`, `nama_karyawan`, `nik`, p.id_karyawan, p.nilai FROM `karyawan` g join 
        `penilaian` p on g.id_karyawan = p.id_karyawan WHERE p.id_periode = ? GROUP BY p.id_karyawan";
    ...
    {
        $query = "SELECT `id_penilaian`, `nama_karyawan`, `nik`, p.id_kriteria , p.id_karyawan, p.nilai, p.normalisasi, p.terbobot FROM `karyawan` g join 
        `penilaian` p on g.id_karyawan = p.id_doses WHERE p.id_periode = ?";
        return $this->db->query($query, $tahun)->result_array();
    }

    public function get_Nilai($id_k, $tahun)
    {
        $query = "SELECT id_penilaian, nilai, id_karyawan, normalisasi, terbobot FROM penilaian WHERE id_kriteria = ? AND id_periode = ?";
        return $this->db->query($query, array($id_k, $tahun))->result_array();
    }

    public function get_Nilai2($id_k, $id_g, $tahun)
    {
        $query = "SELECT id_penilaian, nilai, id_karyawan, normalisasi, terbobot FROM penilaian WHERE id_kriteria = ? AND id_karyawan = ? AND id_periode = ?";
        return $this->db->query($query, array($id_k, $id_g, $tahun))->row_array();
    }

    public function update_Normalisasi($normalisasi, $id_k, $id_g, $tahun)
    {
        $query = "UPDATE penilaian SET normalisasi = ? WHERE id_kriteria = ? AND id_karyawan = ? AND id_periode = ?";
        $this->db->query($query, array($normalisasi, $id_k, $id_g, $tahun));
    }

    public function update_Terbobot($terbobot, $id_k, $id_g, $tahun)
    {
        $query = "UPDATE penilaian SET terbobot = ? WHERE id_kriteria = ? AND id_karyawan = ? AND id_periode = ?";
